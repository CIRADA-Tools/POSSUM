...
names= df.loc[(df["flux_peak"] > fluxmin) & (df["flux_peak"] < fluxmax)]["component_name"].values
cut_ra = df.loc[(df["flux_peak"] > fluxmin) & (df["flux_peak"] < fluxmax)]["ra_deg_cont"].values
cut_dec = df.loc[(df["flux_peak"] > fluxmin) & (df["flux_peak"] < fluxmax)]["dec_deg_cont"].values
specindex = df.loc[(df["flux_peak"] > fluxmin) & (df["flux_peak"] < fluxmax)]["spectral_index"].values
...
for source in range(0, numSources):
    print("Making plot for "+str(names[source])+" ("+str(source+1)+" of "+ str(numSources+1) +").")
...
makeHTML(path, sb, base, fluxmin, fluxmax, min_dist, max_dist)